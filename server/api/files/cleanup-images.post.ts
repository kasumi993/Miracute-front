...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Extract all used image URLs and filenames from products
      ...
      if (product.preview_images && Array.isArray(product.preview_images)) {
        product.preview_images.forEach(url => {
          if (typeof url === 'string') {
            ...
            // Extract filename from URL for safer comparison
            try {
    ...
    files.forEach(file => {
      ...
      console.log(`  URL match: ${usedImageUrls.has(publicUrl)}`)
      console.log(`  Filename match: ${usedFilenames.has(file.name)}`)

      // Check both URL and filename for safety
      const isUsed = usedImageUrls.has(publicUrl) || usedFilenames.has(file.name)

      if (!isUsed) {
        console.log('  -> MARKED FOR DELETION')
        orphanedFiles.push(file.name)
      } else {
        console.log('  -> KEEPING (in use)')
      }
    })

 ...
 */
export async function isAdminUser(userId: string, event?: H3Event): Promise<boolean> {
  try {
    // If no event provided, create a minimal one - this is for backward compatibility
    // But preferably pass the actual event for proper context
    const supabase = event
      ? serverSupabaseServiceRole<Database>(event)
      : serverSupabaseServiceRole<Database>({} as H3Event)
